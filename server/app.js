...
const convertBlogDbObjectAPI1 = (objectItem) => {
  return {
    blogId: objectItem.id,
    title: objectItem.title,
    author: objectItem.author,
    content: objectItem.content,
    publictaionDate: objectItem.publication_date,
    imgUrl:objectItem.imgUrl
  };
};
app.get("/blogs/", async (request, response) => {
  const getBlogsListQuery = `select * from blog;`;
  const getBlogsListQueryResponse = await database.all(getBlogsListQuery);
  response.send(
    getBlogsListQueryResponse.map((eachBlog) =>
      convertBlogDbObjectAPI1(eachBlog)
    )
  );
});
...
//   const getBlogsListByIdQuery = `select * from state where state_id = ${blogId};`;
//   const getBlogsListByIdQueryResponse = await database.get(
//     getBlogsListByIdQuery
//   );
...
app.post("/blogss/", async (request, response) => {
  const { title,author,publicationDate,content,imgUrl } = request.body;
  const createBlogQuery = `insert into blogs(id,title,author,publication_date,content,imgUrl)
    values('${title}','${author}','${publicationDate}','${content}','${imgUrl}');`;
  const createBlogQueryResponse = await database.run(createBlogQuery);
  response.send("Blog Successfully Added");
});
